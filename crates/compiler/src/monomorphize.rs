...
impl MonomorphizationContext {
    ...
    fn substitute_signature(
        ...
        new_sig.returns = new_sig.returns.iter()
    ...
    fn substitute_type(
        ...
        match ty {
            ...
            // Recursive cases
            HirType::Ptr(inner) => {
                HirType::Ptr(Box::new(self.substitute_type(inner, type_subst, const_subst)))
            }
            ...
            HirType::Struct(struct_ty) => {
                ...
                new_struct.fields = new_struct.fields.iter()
            ...
            HirType::Function(func_ty) => {
                ...
                new_func.params = new_func.params.iter()
                    .map(|p| self.substitute_type(p, type_subst, const_subst))
                    .collect();
                new_func.returns = new_func.returns.iter()
            ...
            HirType::Closure(closure_ty) => {
                ...
                new_closure.function_type.params = new_closure.function_type.params.iter()
                    .map(|p| self.substitute_type(p, type_subst, const_subst))
                    .collect();
                new_closure.function_type.returns = new_closure.function_type.returns.iter()
                    .map(|r| self.substitute_type(r, type_subst, const_subst))
                    .collect();
...
pub fn monomorphize_module(module: &mut HirModule) -> CompilerResult<()> {
    ...
    let generic_funcs: Vec<_> = module.functions.iter()
        .filter(|(_, f)| !f.signature.type_params.is_empty() || !f.signature.const_params.is_empty())
        .map(|(id, f)| (*id, f.clone()))
        .collect();
    ...
    for (func_id, type_args, const_args, call_site) in instantiations {
        ...
        if ctx.generic_functions.contains_key(&func_id) {
            ...
            let instance_id = ctx.get_or_create_instance(func_id, type_args.clone(), const_args.clone())?;
    ...
    for func in module.functions.values_mut() {
        for block in func.blocks.values_mut() {
            for (inst_idx, inst) in block.instructions.iter_mut().enumerate() {
                if let HirInstruction::Call { callee, type_args, .. } = inst {
...
fn find_instantiations(module: &HirModule) -> CompilerResult<Vec<(HirId, Vec<HirType>, Vec<HirConstant>, (HirId, usize))>> {
    ...
    for func in module.functions.values() {
        for block in func.blocks.values() {
            for (inst_idx, inst) in block.instructions.iter().enumerate() {
                if let HirInstruction::Call { callee, type_args, const_args, .. } = inst {
...
mod tests {
    ...
    fn test_monomorphization_key_deduplication() {
        ...
        let instance1 = ctx.get_or_create_instance(
            func_id,
            vec![HirType::I32],
            vec![],
        ).unwrap();

        let instance2 = ctx.get_or_create_instance(
            func_id,
            vec![HirType::I32],
            vec![],
        ).unwrap();
        ...
        assert_eq!(instance1, instance2, "Same type args should return same instance");

        // Create instance with different type args
        let instance3 = ctx.get_or_create_instance(
            func_id,
            vec![HirType::F64],
            vec![],
        ).unwrap();

        assert_ne!(instance1, instance3, "Different type args should return different instance");
    }
    ...
    fn test_monomorphize_function_basic() {
        ...
        assert!(instance.signature.type_params.is_empty(), "Should have no type params");
        assert_eq!(instance.signature.params[0].ty, HirType::I32, "Parameter should be i32");
        assert_eq!(instance.signature.returns[0], HirType::I32, "Return should be i32");
    }
    ...
    fn test_const_generic_monomorphization() {
        ...
        let instance = ctx.monomorphize_function(
            &func,
            &[HirType::I32],
            &[HirConstant::U64(5)]
        );
        ...
        assert!(instance.signature.const_params.is_empty(), "Should have no const params");
        assert!(instance.signature.type_params.is_empty(), "Should have no type params");
    ...
    fn test_const_generic_deduplication() {
        ...
        assert_eq!(instance1, instance2, "Same type+const args should reuse instance");

        // Create instance with different const arg
        let instance3 = ctx.get_or_create_instance(
            func_id,
            vec![HirType::I32],
            vec![HirConstant::U64(20)],
        ).unwrap();

        // Should be different instance
        assert_ne!(instance1, instance3, "Different const args should create different instance");

        // Create instance with different type arg
        let instance4 = ctx.get_or_create_instance(
            func_id,
            vec![HirType::F64],
            vec![HirConstant::U64(10)],
        ).unwrap();
        ...
        // Should be different instance
        assert_ne!(instance1, instance4, "Different type args should create different instance");
    }
    ...
    fn test_nested_generic_types() {
        ...
        // Represents: *[T; 5]
        let nested_ty = HirType::Ptr(Box::new(
            HirType::Array(Box::new(HirType::Opaque(t_param)), 5)
        ));
    ...
    fn test_deeply_nested_generic_types() {
        ...
        // Represents: **[*T; 3]
        let deeply_nested = HirType::Ptr(Box::new(
            HirType::Ptr(Box::new(
                HirType::Array(
    ...
    fn test_generic_function_with_nested_returns() {
        ...
        let signature = HirFunctionSignature {
            ...
            returns: vec![
                // Return *[T; 10]
                HirType::Ptr(Box::new(
                    HirType::Array(Box::new(HirType::Opaque(t_param)), 10)
                ))
            ],
            type_params: vec![HirTypeParam {
                name: t_param,
    ...
    fn test_monomorphize_module_actually_works() {
        ...
        if let HirInstruction::Call { type_args, .. } = &block_before.instructions[0] {
            assert!(!type_args.is_empty(), "Should have type_args before monomorphization");
        }

        // RUN MONOMORPHIZATION - THE ACTUAL FUNCTION WE'RE TESTING
        let result = monomorphize_module(&mut module);
        assert!(result.is_ok(), "Monomorphization should succeed: {:?}", result.err());

        // AFTER: 3 functions (generic + i32 instance + caller)
        assert_eq!(module.functions.len(), 3, "Should create monomorphized instance");
        ...
        if let HirInstruction::Call { callee, type_args, .. } = &block_after.instructions[0] {
            ...
            if let HirCallable::Function(called_id) = callee {
                ...
                let instance = module.functions.get(called_id)
                    .expect("Monomorphized instance should exist");

                assert!(instance.signature.type_params.is_empty(), "Instance should have no type params");
                assert_eq!(instance.signature.params[0].ty, HirType::I32, "Param should be i32");
                assert_eq!(instance.signature.returns[0], HirType::I32, "Return should be i32");
