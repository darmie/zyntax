...
use crate::hir::{HirModule, HirFunction, HirBlock, HirId, HirInstruction, HirValue};
...
impl AnalysisRunner {
    ...
    fn compute_use_def_sets(&self, func: &HirFunction) -> (HashMap<HirId, HashSet<HirId>>, HashMap<HirId, HashSet<HirId>>) {
    ...
    fn collect_instruction_uses(&self, inst: &HirInstruction, uses: &mut HashSet<HirId>, defs: &HashSet<HirId>) {
        match inst {
            HirInstruction::Binary { left, right, .. } => {
                if !defs.contains(left) { uses.insert(*left); }
                if !defs.contains(right) { uses.insert(*right); }
            }
            HirInstruction::Unary { operand, .. } => {
                if !defs.contains(operand) { uses.insert(*operand); }
            }
            HirInstruction::Load { ptr, .. } => {
                if !defs.contains(ptr) { uses.insert(*ptr); }
            }
            HirInstruction::Store { value, ptr, .. } => {
                if !defs.contains(value) { uses.insert(*value); }
                if !defs.contains(ptr) { uses.insert(*ptr); }
            }
            HirInstruction::Call { callee, args, .. } => {
                ...
                for arg in args {
                    if !defs.contains(arg) { uses.insert(*arg); }
                }
            }
            _ => {}
        }
    ...
    fn collect_terminator_uses(&self, term: &crate::hir::HirTerminator, uses: &mut HashSet<HirId>, defs: &HashSet<HirId>) {
        match term {
            ...
            crate::hir::HirTerminator::CondBranch { condition, .. } => {
                if !defs.contains(condition) { uses.insert(*condition); }
            }
            ...
            for inst in &block.instructions {
                match inst {
                    HirInstruction::Load { result, ptr, .. } => {
                        ...
                        reads.entry(*result).or_insert_with(HashSet::new).insert(loc);
                    }
