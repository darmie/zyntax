...
fn test_arc_instrumentation() {
    ...
    block.set_terminator(HirTerminator::Return { values: vec![malloc_result] });
...
fn test_escape_analysis() {
    ...
    let block = func.blocks.get_mut(&func.entry_block).unwrap();
    block.set_terminator(HirTerminator::Return { values: vec![heap_value] });
...
fn test_memory_context_reference_counting() {
    ...
    // Track allocations
    ctx.track_allocation(value1, AllocationInfo {
    ...
    ctx.track_allocation(value2, AllocationInfo {
...
fn test_stack_allocation_optimization() {
    ...
    ctx.escape_info.insert(stack_value, EscapeInfo {
        escapes: false,
    ...
    ctx.escape_info.insert(heap_value, EscapeInfo {
        escapes: true,
        escape_targets: HashSet::new(),
        is_returned: true,
        stored_in_heap: false,
...
fn test_drop_type_checking() {
    ...
    let array_of_ptrs = HirType::Array(
        Box::new(HirType::Ptr(Box::new(HirType::I32))),
        10
    );
