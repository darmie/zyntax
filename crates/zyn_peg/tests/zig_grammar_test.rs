...
fn test_parse_zig_zyn_grammar() {
    ...
    match result {
        Ok(pairs) => {
            ...
            assert!(grammar.language.file_extensions.contains(&".zig".to_string()));
            println!("✓ Language info: {} v{}", grammar.language.name, grammar.language.version);
            ...
            if !grammar.type_helpers.code.is_empty() {
                println!("✓ Type helpers present ({} chars)", grammar.type_helpers.code.len());
            }
        }
        Err(e) => {
...
fn test_zig_zyn_has_required_rules() {
    ...
    let rule_names: std::collections::HashSet<_> = grammar.rules.iter()
        .map(|r| r.name.as_str())
        .collect();
...
fn test_generate_pest_grammar_from_zig_zyn() {
    ...
    println!("✓ Generated pest grammar ({} chars)", generated.pest_grammar.len());
    ...
    println!("{}", &generated.pest_grammar[..500.min(generated.pest_grammar.len())]);
    println!("---");
