...
fn test_effect_composition_workflow() {
    ...
    let alternative_result = system.compose_effects(
        io_effects,
        memory_effects,
...
fn test_complete_effect_system_workflow() {
    ...
    let read_func = InternedString::from_symbol(string_interner::Symbol::try_from_usize(2).unwrap());
    let write_func = InternedString::from_symbol(string_interner::Symbol::try_from_usize(3).unwrap());
    ...
    write_sig.output_effects.add_effect(system.create_state_effect());
...
fn test_effect_error_handling() {
    ...
    let span = Span::new(0, 50);
    let compatibility_result = system.check_effect_compatibility(
        &non_empty_effects, 
        &empty_effects, 
        span
    );
