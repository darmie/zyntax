...
fn create_test_program_with_variable(var_name: &str, var_type: Type) -> TypedProgram {
    let name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(1).unwrap());
...
fn create_test_function() -> TypedProgram {
    let span = Span::new(0, 10);
    let func_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(2).unwrap());
...
fn test_linear_type_checker_creation() {
    ...
    assert!(result.is_ok(), "Linear type checker should handle non-linear types");
}
...
fn test_linear_type_checker_with_function() {
    ...
    let result = checker.check_program(&program);
    assert!(result.is_ok(), "Linear type checker should handle simple functions");
}
...
fn test_linear_type_checker_empty_program() {
    ...
    let result = checker.check_program(&program);
    assert!(result.is_ok(), "Linear type checker should handle empty programs");
}
...
fn test_program_with_multiple_declarations() {
    ...
    let result = checker.check_program(&program);
    assert!(result.is_ok(), "Linear type checker should handle multiple variable declarations");
}
...
fn test_linear_type_info_creation() {
    ...
    assert_eq!(file_handle_info.resource_kind, Some(ResourceKind::FileHandle));
...
fn test_complex_program_structure() {
    let mut checker = LinearTypeChecker::new();
    let span = Span::new(0, 10);
    let func_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(1).unwrap());
    ...
    let result = checker.check_program(&program);
    assert!(result.is_ok(), "Linear type checker should handle complex program structures");
}