...
use zyntax_typed_ast::type_registry::{Type, ConstValue, TypeVar, TypeVarId, TypeVarKind};
use zyntax_typed_ast::{PrimitiveType, NullabilityKind};
...
fn test_refinement_type_well_formedness() {
    ...
    let var_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(1).unwrap());
    ...
    assert!(result.is_ok(), "Positive integer refinement should be well-formed");
}
...
fn test_refinement_with_unbound_variable() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let var_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(2).unwrap());
    let unbound_var = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(3).unwrap());
    ...
    match result {
        Err(DependentTypeError::UnboundVariable { var, .. }) => {
            assert_eq!(var, unbound_var, "Should detect the correct unbound variable");
        },
...
fn test_dependent_function_scope() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let param_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(4).unwrap());
    ...
    let dep_function = DependentType::DependentFunction {
        ...
        return_type: Box::new(DependentType::refinement(
            ...
            RefinementPredicate::Comparison {
                op: ComparisonOp::Greater,
                left: Box::new(RefinementExpr::Variable(
                    InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(5).unwrap())
                )),
    ...
    assert!(result.is_ok(), "Dependent function should be well-formed with parameter in scope");
}
...
fn test_dependent_pair_scoping() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let first_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(6).unwrap());
    let second_var = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(7).unwrap());
    ...
    assert!(result.is_ok(), "Dependent pair should allow second to reference first");
}
...
fn test_path_dependent_type_resolution() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let module_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(8).unwrap());
    let type_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(9).unwrap());
    ...
    assert!(result.is_ok(), "Path-dependent type should pass basic well-formedness");
}
...
fn test_recursive_type_detection() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let var_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(10).unwrap());
    ...
    assert!(result.is_ok(), "Simple recursive type should be well-formed");
}
...
fn test_universal_quantification() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let var_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(11).unwrap());
...
fn test_existential_quantification() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let var_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(13).unwrap());
...
fn test_indexed_family_application() {
    let mut checker = DependentTypeChecker::new();
    let span = Span::new(0, 10);
    let family_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(14).unwrap());
...
fn test_complex_refinement_predicates() {
    let var_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(16).unwrap());
