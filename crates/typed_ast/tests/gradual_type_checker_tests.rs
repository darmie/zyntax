...
use zyntax_typed_ast::type_registry::{Type, PrimitiveType, NullabilityKind};
...
fn test_gradual_compatibility_any_type() {
    ...
    assert!(checks.is_empty(), "Any type should not require runtime checks");
    ...
    assert!(checks.is_empty(), "Any type should accept any value without checks");
}
...
fn test_dynamic_to_static_conversion() {
    ...
    assert!(!checks.is_empty(), "Dynamic to static should require runtime checks");
...
fn test_nullable_type_checks() {
    ...
    assert!(checks.len() >= 2, "Nullable should generate null check and type check");
    ...
    let has_type_assertion = checks.iter().any(|check| {
        matches!(check.check_kind, RuntimeCheckKind::TypeAssertion)
    });
    assert!(has_type_assertion, "Should have type assertion for inner type");
}
...
fn test_interface_duck_typing() {
    ...
    assert!(!checks.is_empty(), "Interface should require duck type checks");
    
    let has_duck_check = checks.iter().any(|check| {
        matches!(check.check_kind, RuntimeCheckKind::DuckTypeCheck { .. })
    });
    assert!(has_duck_check, "Should generate duck type check for interface");
}
...
fn test_static_to_dynamic_no_checks() {
    ...
    assert!(checks.is_empty(), "Dynamic should accept static types without checks");
}
...
fn test_unknown_type_handling() {
    ...
    assert!(!checks.is_empty(), "Unknown type should require runtime checks");
}
...
fn test_exact_type_match_no_checks() {
    ...
    assert!(checks.is_empty(), "Exact type match should not require runtime checks");
}
...
fn test_complex_type_with_multiple_checks() {
    ...
    assert!(checks.len() >= 2, "Complex type should generate multiple checks");
    
    // Should have both null check and duck type check
    let has_null_check = checks.iter().any(|c| matches!(c.check_kind, RuntimeCheckKind::NullCheck));
    let has_duck_check = checks.iter().any(|c| matches!(c.check_kind, RuntimeCheckKind::DuckTypeCheck { .. }));
