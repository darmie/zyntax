...
use zyntax_typed_ast::type_registry::{Type, PrimitiveType, NullabilityKind, TypeVarKind, Variance, TypeId, TypeParam, Visibility, MethodSig, ParamDef};
...
fn test_nominal_subtyping_inheritance() {
    ...
    let object_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(1).unwrap());
    ...
    let string_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(2).unwrap());
    ...
    assert!(checker.is_subtype(&string_type, &object_type).unwrap(), "String should be subtype of Object");
    
    // Object is NOT a subtype of String
    assert!(!checker.is_subtype(&object_type, &string_type).unwrap(), "Object should not be subtype of String");
    
    // A type is always a subtype of itself
    assert!(checker.is_subtype(&string_type, &string_type).unwrap(), "String should be subtype of itself");
}
...
fn test_circular_inheritance_detection() {
    ...
    let type_a_def = TypeDefinition {
        id: type_a_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(3).unwrap()),
        kind: TypeKind::Class,
        type_params: vec![],
        super_type: None,
        interfaces: vec![],
    ...
    let type_b_def = TypeDefinition {
        id: type_b_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(4).unwrap()),
        kind: TypeKind::Class,
        type_params: vec![],
        super_type: Some(Type::Named {
            id: type_a_id,
...
fn test_method_resolution() {
    ...
    let method_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(5).unwrap());
    ...
    let class_def = TypeDefinition {
        id: class_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(6).unwrap()),
        kind: TypeKind::Class,
        type_params: vec![],
        super_type: None,
        interfaces: vec![],
    ...
    let resolved = checker.resolve_method(&class_type, method_name, &[]).unwrap();
    ...
    let non_existent = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(7).unwrap());
    let not_found = checker.resolve_method(&class_type, non_existent, &[]).unwrap();
...
fn test_trait_implementation() {
    ...
    let trait_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(8).unwrap());
    let method_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(9).unwrap());
    ...
    let type_def = TypeDefinition {
        id: type_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(10).unwrap()),
        kind: TypeKind::Class,
        type_params: vec![],
        super_type: None,
        interfaces: vec![],
    ...
    assert!(checker.implements_trait(type_id, trait_id).unwrap(), "Type should implement trait");
    ...
    assert!(checker.is_subtype(&type_instance, &trait_instance).unwrap(), "Type should be subtype of implemented trait");
}
...
fn test_virtual_method_override_validation() {
    ...
    let method_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(11).unwrap());
    let param_name = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(12).unwrap());
    ...
    let result = checker.check_virtual_method_override(&base_method, &valid_override, base_id, derived_id);
    ...
    let result = checker.check_virtual_method_override(&base_method, &invalid_params, base_id, derived_id);
    match result {
    ...
    let result = checker.check_virtual_method_override(&base_method, &wrong_return, base_id, derived_id);
    match result {
        Err(NominalTypeError::CovarianceViolation { param_or_return, .. }) => {
...
fn test_variance_in_generic_types() {
    ...
    let t_param = InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(13).unwrap());
    
    let list_def = TypeDefinition {
        id: list_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(14).unwrap()),
        kind: TypeKind::Class,
    ...
    let animal_def = TypeDefinition {
        id: animal_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(15).unwrap()),
        kind: TypeKind::Class,
        type_params: vec![],
        super_type: None,
        interfaces: vec![],
    ...
    let dog_def = TypeDefinition {
        id: dog_id,
        name: InternedString::from_symbol(string_interner::DefaultSymbol::try_from_usize(16).unwrap()),
        kind: TypeKind::Class,
        type_params: vec![],
        super_type: Some(Type::Named {
            id: animal_id,
