...
use zyntax_typed_ast::{nominal_type_checker::*, MethodSig, NullabilityKind, PrimitiveType, Type, TypeId, TypeParam, Variance, Visibility};
...
fn test_inheritance_hierarchy() {
    ...
    // Register types
    checker.register_type(object_type).expect("Should register Object");
    checker.register_type(animal_type).expect("Should register Animal");
...
fn test_method_resolution() {
    ...
    checker.register_type(type_def).expect("Should register TestType");
...
fn test_virtual_method_table_generation() {
    ...
    checker.register_type(type_def).expect("Should register VirtualType");
