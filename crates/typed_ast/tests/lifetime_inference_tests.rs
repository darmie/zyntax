...
use zyntax_typed_ast::constraint_solver::{
    Constraint, ConstraintSolver, SolverError
};
use zyntax_typed_ast::source::Span;
use zyntax_typed_ast::type_registry::{
    TypeRegistry, Type, PrimitiveType, Mutability, Lifetime
};
...
fn test_lifetime_basics() {
    ...
    assert!(result.is_ok(), "Lifetime outlives constraint should be solvable");
}
...
fn test_lifetime_cycle_detection() {
    ...
    if let Err(errors) = result2 {
        assert!(errors.iter().any(|e| matches!(e, SolverError::LifetimeCycle { .. })));
...
fn test_type_outlives_lifetime() {
    ...
    solver.add_constraint(Constraint::TypeOutlivesLifetime(
        ref_type,
        lifetime_a,
        span,
    ));
    ...
    let result = solver.solve();
    assert!(result.is_ok(), "Type outlives lifetime constraint should be solvable");
...
fn test_lifetime_constraint_formatting() {
    ...
    // Create constraints
    let outlives_constraint = Constraint::LifetimeOutlives(
        lifetime_a.clone(),
        lifetime_b.clone(),
        Span::new(0, 0),
    );
...
fn test_check_lifetime_constraints() {
    ...
    let result = solver.solve();
    assert!(result.is_ok(), "Transitive lifetime constraints should be solvable");
    
    // Check lifetime constraints are satisfied
    let check_result = solver.check_lifetime_constraints();
    assert!(check_result.is_ok(), "Lifetime constraints should be satisfied");
