...
use zyntax_typed_ast::type_registry::{
    TypeRegistry, Type, PrimitiveType, Mutability
};
...
fn test_implicit_lifetime_generation() {
    ...
    match &ref_type_with_lifetime {
        Type::Reference { lifetime: Some(lt), .. } => {
        ...
        Type::Reference { 
            ...
            match &**inner {
                Type::Reference { 
                    ...
                    assert_ne!(outer_lt.name, inner_lt.name, "Nested lifetimes should be distinct");
                }
...
fn test_function_parameter_lifetime_inference() {
    ...
    match &inferred {
        Type::Function { params, return_type, .. } => {
            ...
            match &params[0].ty {
                Type::Reference { lifetime: Some(lt1), .. } => {
            ...
            match &params[1].ty {
                Type::Reference { lifetime: Some(lt2), .. } => {
            ...
            // Check return type
            match &**return_type {
                Type::Reference { lifetime: Some(lt_ret), .. } => {
...
fn test_explicit_lifetime_preserved() {
    ...
    let explicit_lifetime = zyntax_typed_ast::type_registry::Lifetime::named(
        arena.intern_string("'static")
    );
    ...
    match &result {
        Type::Reference { lifetime: Some(lt), .. } => {
            assert_eq!(lt.name, explicit_lifetime.name, "Explicit lifetime should be preserved");
