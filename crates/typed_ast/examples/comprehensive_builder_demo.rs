...
use zyntax_typed_ast::{
    TypedASTBuilder,
    typed_ast::*,
    Type, TypeId, PrimitiveType, Mutability, Visibility, Variance,
...
fn main() {
    ...
    println!("✅ Regular parameter: {}", builder.arena().resolve_string(regular_param.name).unwrap());
    ...
    println!("✅ Optional parameter with default: {}", builder.arena().resolve_string(optional_param.name).unwrap());
    ...
    let struct_pattern = builder.struct_pattern(
        "Point",
        vec![
            ("x", typed_node(TypedPattern::immutable_var(px_var), Type::Any, span)),
            ("y", typed_node(TypedPattern::immutable_var(py_var), Type::Any, span)),
        ],
        span,
    );
    ...
    let enum_pattern = builder.enum_pattern(
        "Option",
        "Some",
        vec![typed_node(TypedPattern::immutable_var(value_var), Type::Any, span)],
        span,
    );
    ...
    let slice_pattern = builder.slice_pattern(
        vec![typed_node(TypedPattern::immutable_var(first_var), Type::Any, span)],
        Some(typed_node(TypedPattern::Rest { 
            name: Some(middle_var), 
            mutability: Mutability::Immutable 
        }, Type::Any, span)),
        vec![typed_node(TypedPattern::immutable_var(last_var), Type::Any, span)],
        span,
    );
    ...
    let map_pattern = builder.map_pattern(
        vec![
            ("name", typed_node(TypedPattern::immutable_var(n_var), Type::Any, span)),
            ("age", typed_node(TypedPattern::immutable_var(a_var), Type::Any, span)),
        ],
    ...
    // For loop
    let iterable = builder.variable("items", Type::Array { 
