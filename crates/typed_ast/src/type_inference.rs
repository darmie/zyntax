...
use crate::type_registry::{
    Type, TypeVar, TypeVarKind, TypeVarId, PrimitiveType, 
    Mutability, ParamInfo
};
...
pub enum InferenceError {
    ...
    MissingTrait { ty: Type, trait_name: InternedString },
    /// Unknown field
...
impl InferenceContext {
    ...
    pub fn with_options(registry: Box<crate::type_registry::TypeRegistry>, options: InferenceOptions) -> Self {
        Self {
            registry,
            substitutions: HashMap::new(),
            constraints: VecDeque::new(),
    ...
    pub fn unify(&mut self, t1: Type, t2: Type) -> Result<Type, InferenceError> {
        ...
        match (&t1, &t2) {
            ...
            // Function types
            (Type::Function { params: p1, return_type: r1, is_varargs: v1, has_named_params: n1, has_default_params: d1, .. },
             Type::Function { params: p2, return_type: r2, is_varargs: v2, has_named_params: n2, has_default_params: d2, .. }) => {
            ...
            // Reference types
            (Type::Reference { ty: t1, mutability: m1, lifetime: l1, .. },
             Type::Reference { ty: t2, mutability: m2, lifetime: l2, .. }) => {
                if m1 != m2 {
                    return Err(InferenceError::TypeMismatch {
                        expected: (**t1).clone(),
                        found: (**t2).clone(),
    ...
    pub fn apply_substitutions(&self, ty: &Type) -> Type {
        match ty {
            ...
            Type::Named { id, type_args, const_args, variance, nullability } => Type::Named {
                id: *id,
            ...
            Type::Function { params, return_type, is_varargs, has_named_params, has_default_params, async_kind, calling_convention, nullability } => Type::Function {
                params: params.iter()
                    .map(|p| ParamInfo {
                        name: p.name,
            ...
            Type::Array { element_type, size, nullability } => Type::Array {
                element_type: Box::new(self.apply_substitutions(element_type)),
                size: size.clone(),
                nullability: *nullability,
            },
            Type::Reference { ty, mutability, lifetime, nullability } => Type::Reference {
                ty: Box::new(self.apply_substitutions(ty)),
                mutability: *mutability,
                lifetime: lifetime.clone(),
                nullability: *nullability,
            },
            Type::Optional(t) => Type::Optional(
                Box::new(self.apply_substitutions(t))
            ),
    ...
    fn check_subtype(&mut self, sub: &Type, sup: &Type) -> Result<(), InferenceError> {
        ...
        match (&sub, &sup) {
            ...
            (Type::Reference { ty: t1, mutability: Mutability::Immutable, .. },
             Type::Reference { ty: t2, mutability: Mutability::Immutable, .. }) => {
    ...
    fn check_numeric_conversion(&self, from: PrimitiveType, to: PrimitiveType) 
    ...
    fn check_trait_impl(&self, _ty: &Type, _trait_name: InternedString, _args: &[Type]) 
        -> Result<(), InferenceError> {
    ...
    /// Check field access
    fn check_field(&self, _ty: &Type, _field: InternedString, _expected_ty: &Type) 
        -> Result<(), InferenceError> {
    ...
    /// Check method
    fn check_method(&self, _ty: &Type, _method: InternedString, 
                   _params: &[Type], _ret: &Type) -> Result<(), InferenceError> {
    ...
    fn check_reference(&self, ty: &Type, expected_mut: Mutability) 
        -> Result<(), InferenceError> {
        match ty {
    ...
    fn check_indexable(&mut self, container_ty: &Type, index_ty: &Type, element_ty: &Type) 
        -> Result<(), InferenceError> {
        match container_ty {
            Type::Array { element_type, .. } => {
                // Arrays must be indexed by integers
                if !matches!(index_ty, Type::Primitive(PrimitiveType::I32) | Type::TypeVar(_)) {
                    return Err(InferenceError::TypeMismatch {
            ...
            Type::Primitive(PrimitiveType::String) => {
                ...
                if !matches!(element_ty, Type::Primitive(PrimitiveType::Char) | Type::TypeVar(_)) {
                    return Err(InferenceError::TypeMismatch {
    ...
    pub fn add_indexable_constraint(
        ...
        self.add_constraint(Constraint::Indexable(container_type, index_type, element_type));
        Ok(())
    }
}