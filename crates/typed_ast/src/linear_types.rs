...
impl LinearTypeChecker {
    ...
    fn check_expression(&mut self, expr: &TypedExpression) -> LinearTypeResult<()> {
        match expr {
            ...
            TypedExpression::Unary(unary) => {
                self.check_expression(&unary.operand.node)
            },
    ...
    pub fn create_borrow(
        ...
        if let Some(usage) = self.variable_usage.get_mut(&var) {
            ...
            // Register the borrow
            self.borrow_checker.active_borrows.insert(borrow_id, borrow_info);
...
impl LinearTypeInfo {
    ...
    pub fn file_handle(type_id: TypeId) -> Self {
        Self {
            ...
            cleanup_behavior: CleanupBehavior::Automatic(Some(
