...
use crate::arena::{InternedString, AstArena};
use crate::source::Span;
use crate::type_registry::{Type, TypeId, ConstValue, ConstBinaryOp, ConstUnaryOp};
...
impl ConstEvaluator {
    ...
    fn eval_const_value_impl(&mut self, value: &ConstValue) -> ConstEvalResult {
        match value {
            ...
            // Function calls
            ConstValue::FunctionCall { name, args } => {
                self.eval_const_function(*name, args)
            },
        ...
        match &expr.kind {
            ...
            ConstExprKind::Call { func, args } => {
                let arg_vals: Result<Vec<_>, _> = args.iter()
                    .map(|arg| self.eval_const_expr(arg))
                    .collect();
        ...
        match ty {
            Type::Primitive(prim) => {
                let size = match prim {
                    ...
                    _ => return Err(ConstEvalError::UnsupportedOperation("sizeof for this type".to_string())),
                };
            ...
            Type::Named { id, .. } => {
                if let Some(layout) = self.context.type_context.get(id) {
                    ...
                    Err(ConstEvalError::UnsupportedOperation("sizeof for unknown type".to_string()))
                }
            },
            ...
            _ => Err(ConstEvalError::UnsupportedOperation(format!("sizeof for {:?}", ty))),
        }
    }
        ...
        match ty {
            Type::Primitive(prim) => {
                let align = match prim {
                    ...
                    PrimitiveType::I16 | PrimitiveType::U16 => 2,
                    PrimitiveType::I32 | PrimitiveType::U32 | PrimitiveType::F32 | PrimitiveType::Char => 4,
                    PrimitiveType::I64 | PrimitiveType::U64 | PrimitiveType::F64 | 
                    PrimitiveType::ISize | PrimitiveType::USize => 8,
                    PrimitiveType::String => 8, // Pointer alignment
                    _ => return Err(ConstEvalError::UnsupportedOperation("alignof for this type".to_string())),
                };
            ...
            Type::Named { id, .. } => {
                if let Some(layout) = self.context.type_context.get(id) {
                    ...
                    Err(ConstEvalError::UnsupportedOperation("alignof for unknown type".to_string()))
                }
            },
            
            _ => Err(ConstEvalError::UnsupportedOperation(format!("alignof for {:?}", ty))),
        }
    }
    
    /// Evaluate binary operations
    fn eval_binary_op(&self, op: ConstBinaryOp, left: &ConstValue, right: &ConstValue) -> ConstEvalResult {
        match (left, right) {
            // Integer arithmetic
            (ConstValue::Int(l), ConstValue::Int(r)) => {
                match op {
                    ConstBinaryOp::Add => l.checked_add(*r).map(ConstValue::Int).ok_or(ConstEvalError::IntegerOverflow),
                    ConstBinaryOp::Sub => l.checked_sub(*r).map(ConstValue::Int).ok_or(ConstEvalError::IntegerOverflow),
                    ConstBinaryOp::Mul => l.checked_mul(*r).map(ConstValue::Int).ok_or(ConstEvalError::IntegerOverflow),
                    ConstBinaryOp::Div => {
                        if *r == 0 {
                            Err(ConstEvalError::DivisionByZero)
                        } else {
                            l.checked_div(*r).map(ConstValue::Int).ok_or(ConstEvalError::IntegerOverflow)
                        }
                    },
                    ConstBinaryOp::Mod => {
                        if *r == 0 {
                    ...
                    _ => Err(ConstEvalError::UnsupportedOperation(format!("integer {:?}", op))),
                }
            },
            ...
            (ConstValue::UInt(l), ConstValue::UInt(r)) => {
                match op {
                    ConstBinaryOp::Add => l.checked_add(*r).map(ConstValue::UInt).ok_or(ConstEvalError::IntegerOverflow),
                    ConstBinaryOp::Sub => l.checked_sub(*r).map(ConstValue::UInt).ok_or(ConstEvalError::IntegerOverflow),
                    ConstBinaryOp::Mul => l.checked_mul(*r).map(ConstValue::UInt).ok_or(ConstEvalError::IntegerOverflow),
                    ConstBinaryOp::Div => {
                        if *r == 0 {
                            Err(ConstEvalError::DivisionByZero)
                        } else {
                    ...
                    ConstBinaryOp::Gt => Ok(ConstValue::Bool(l > r)),
                    ConstBinaryOp::Ge => Ok(ConstValue::Bool(l >= r)),
                    _ => Err(ConstEvalError::UnsupportedOperation(format!("unsigned integer {:?}", op))),
                }
            },
            ...
            (ConstValue::Bool(l), ConstValue::Bool(r)) => {
                match op {
                    ...
                    ConstBinaryOp::Eq => Ok(ConstValue::Bool(l == r)),
                    ConstBinaryOp::Ne => Ok(ConstValue::Bool(l != r)),
                    _ => Err(ConstEvalError::UnsupportedOperation(format!("boolean {:?}", op))),
                }
            },
    ...
    fn eval_unary_op(&self, op: ConstUnaryOp, operand: &ConstValue) -> ConstEvalResult {
        match operand {
            ConstValue::Int(val) => {
                match op {
                    ConstUnaryOp::Neg => val.checked_neg().map(ConstValue::Int).ok_or(ConstEvalError::IntegerOverflow),
            ...
            ConstValue::UInt(val) => {
                match op {
                    ...
                    _ => Err(ConstEvalError::UnsupportedOperation(format!("unsigned {:?}", op))),
                }
            },
            ...
            _ => Err(ConstEvalError::UnsupportedOperation(format!("{:?} on {:?}", op, operand))),
        }
    }
    ...
    fn eval_const_function(&mut self, name: InternedString, args: &[ConstValue]) -> ConstEvalResult {
    ...
    fn eval_literal(&self, lit: &Literal) -> ConstEvalResult {
        match lit {
            ...
            Literal::Float(_) => Err(ConstEvalError::UnsupportedOperation("float literals in const context".to_string())),
            ...
            Literal::Null => Err(ConstEvalError::UnsupportedOperation("null in const context".to_string())),
            Literal::Unit => Err(ConstEvalError::UnsupportedOperation("unit in const context".to_string())),
            Literal::Placeholder => Err(ConstEvalError::UnsupportedOperation("placeholder literal".to_string())),
        }
    }
    
    /// Check if a const constraint is satisfied
    pub fn check_constraint(&mut self, value: &ConstValue, constraint: &ConstConstraint) -> Result<bool, ConstEvalError> {
        match constraint {
            ...
            ConstConstraint::Predicate(pred) => {
                ...
                Err(ConstEvalError::UnsupportedOperation("predicate constraints".to_string()))
            },
